...
class STTNInpaint:
    def __init__(self, device, model_path):
        print(f"[STTNInpaint.__init__] device={device}, model_path={model_path}, exists={os.path.exists(model_path)}")
        self.device = device
        # 1. 创建InpaintGenerator模型实例并装载到选择的设备上
        self.model = InpaintGenerator().to(self.device)
        print("[STTNInpaint.__init__] Model created, loading weights...")
        # 2. 载入预训练模型的权重，转载模型的状态字典
        self.model.load_state_dict(torch.load(model_path, map_location='cpu')['netG'])
        print("[STTNInpaint.__init__] Weights loaded, setting eval mode...")
        # 3. # 将模型设置为评估模式
        self.model.eval()
        print("[STTNInpaint.__init__] Done")
        # 模型输入用的宽和高
