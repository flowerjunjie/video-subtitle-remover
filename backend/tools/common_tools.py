...
def merge_big_file_if_not_exists(dir, file, man_filename = None):
    print(f"[merge_big_file_if_not_exists] dir={dir}, file={file}, files_in_dir={os.listdir(dir) if os.path.exists(dir) else 'DIR_NOT_FOUND'}")
    if file not in os.listdir(dir):
        print(f"[merge_big_file_if_not_exists] Merging {file} from chunks...")
        fs = Filesplit()
        if man_filename is not None:
            fs.man_filename = man_filename
        fs.merge(input_dir=dir)
        print(f"[merge_big_file_if_not_exists] Merge complete, result files={os.listdir(dir)}")
