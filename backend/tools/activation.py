...
def activate_code(code: str, machine_id: str = None) -> Tuple[bool, str, Optional[Dict]]:
    ...
    try:
        ...
        url = f"{app_config.VSR_API_SERVER}/api/codes/activate"
        payload = json.dumps({"code": code, "machine_id": machine_id}).encode("utf-8")
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            if result.get("success"):
                ...
                save_activation_status({
                    ...
                    "days_remaining": result.get("months", 1) * 30
                })
                return True, "激活成功", result
            else:
        ...
        try:
            ...
            return False, error_body.get("message", "激活失败"), None
        except Exception:
            return False, f"激活失败 (HTTP {e.code})", None
    except Exception as e:
        # 网络异常，回退本地激活（不推荐但保证可用）
        return _activate_code_local(code, machine_id)
...
def check_activation_status() -> Tuple[bool, int]:
    """
    检查激活状态（本地优先，服务端实时验证兜底）
    返回：(是否激活且未过期, 剩余天数)
    """
    # 优先服务端实时验证
    online_result = check_activation_status_online()
...
def check_activation_status_online() -> Optional[Tuple[bool, int]]:
    """
    服务端实时验证激活状态
    返回 None 表示网络异常走本地验证，否则返回 (是否有效, 剩余天数)
    """
    try:
        import urllib.request
        import urllib.error
        ...
        machine_id = get_machine_id()
        url = f"{app_config.VSR_API_SERVER}/api/codes/verify"
        ...
        with urllib.request.urlopen(req, timeout=5) as resp:
            ...
            if result.get("valid"):
                return True, result.get("days_remaining", 0)
            else:
                return False, 0
...
def _activate_code_local(code: str, machine_id: str) -> Tuple[bool, str, Optional[Dict]]:
    """本地激活（网络异常时的兜底逻辑）"""
    codes_data = load_activation_codes()
    found_entry = None
    found_index = -1
