...
class ModelConfig:
    def __init__(self):
        _base = BASE_DIR
        self.LAMA_MODEL_DIR = os.path.join(_base, 'backend', 'models', 'big-lama')
        self.STTN_AUTO_MODEL_PATH = os.path.join(_base, 'backend', 'models', 'sttn-auto', 'infer_model.pth')
        self.STTN_DET_MODEL_PATH = os.path.join(_base, 'backend', 'models', 'sttn-det', 'sttn.pth')
        self.PROPAINTER_MODEL_DIR = os.path.join(_base, 'backend', 'models', 'propainter')
        if config.subtitleDetectMode.value == SubtitleDetectMode.PP_OCRv5_MOBILE:
            self.DET_MODEL_DIR = os.path.join(_base, 'backend', 'models', 'V5', 'ch_det_fast')
        elif config.subtitleDetectMode.value == SubtitleDetectMode.PP_OCRv5_SERVER:
            self.DET_MODEL_DIR = os.path.join(_base, 'backend', 'models', 'V5', 'ch_det')
        else:
