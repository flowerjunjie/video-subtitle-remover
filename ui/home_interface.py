...
class HomeInterface(QWidget):
    ...
    def remover_process(queue, video_path, output_path, options):
        ...
        Args:
            ...
            options: 选项
        """
        print(f"[remover_process] START video_path={video_path}")
        sr = None
        try:
            print("[remover_process] Importing SubtitleRemover...")
            from backend.main import SubtitleRemover
            print("[remover_process] Creating SubtitleRemover instance...")
            sr = SubtitleRemover(video_path, True)
            print(f"[remover_process] SubtitleRemover created, setting output_path={output_path}")
            ...
            print("[remover_process] Calling sr.run()...")
            sr.run()
            print("[remover_process] sr.run() returned normally")
        except Exception as e:
            traceback.print_exc()
    ...
    def run_subtitle_remover_process(self, video_path, output_path, options):
        ...
        try:
            ...
            timeout_seconds = 1800  # 30 minutes - accounts for init time + processing at ~1fps
            wait_interval = 2
            elapsed = 0
            ...
            if process.is_alive():
                ...
                self.append_log_signal.emit(["Processing timeout, terminated"])
                self.task_status_signal.emit(self.current_processing_task_index, TaskStatus.FAILED)
